...
st.markdown(
    """
    <style>
    .stApp { background: #0b0e11 !important; color: #eaecef !important; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    h1, h2, h3, h4, h5, h6, p, span, label, div { color: #eaecef !important; }
    .ticker-card { background: linear-gradient(135deg, #181a20 0%, #0b0e11 100%); border: 1px solid #23272e; border-radius: 8px; padding: 12px 15px; text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.4); }
    .broker-card { background: linear-gradient(135deg, #181a20 0%, #12161c 100%); border: 1px solid #23272e; border-top: 3px solid #fcd535; padding: 30px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.6); }
    .stTextInput>div>div>input, .stNumberInput>div>div>input { background-color: #0b0e11 !important; color: #ffffff !important; border: 1px solid #2b313a !important; border-radius: 6px !important; height: 42px !important; }
    .stButton>button { width: 100%; border-radius: 6px; font-weight: 700; height: 42px; background: #fcd535; color: #0b0e11; border: none; }
    .stButton>button:hover { background: #f0b90b !important; color: #0b0e11 !important; }
    [data-testid="stSidebar"] { background-color: #12161c !important; border-right: 1px solid #23272e; }
    </style>
""",
...
# --- ROBUST AUTHENTICATION & LOGIN FIX ---
def verify_user_credentials(input_val, password):
  try:
    ...
    # फ्लेक्सिबल क्वेरी जो मोबाइल नंबर, ईमेल या यूजरनेम तीनों से लॉगिन हैंडल करेगी
    cursor.execute(
        """
    ...
    if res:
      stored_pass, name, username, tier, balance, real_email = res
      if str(stored_pass) == str(password):
        return {
            "success": True,
            "email": real_email,
            "name": name,
            "username": username if username else "trader",
            "tier": tier if tier else "VIP Platinum",
            "balance": balance if balance is not None else 10000.0,
        }
